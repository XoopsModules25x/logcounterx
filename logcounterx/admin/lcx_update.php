...
function xoops_module_update_logcounterx(&$module, $prev_version) {
	...
	if ($xoopsDB->getRowsNum($res) != 24) {
		...
		while (list($H, $C) = $xoopsDB->fetchRow($res1)) { $HData[intval($H)] = intval($C); }
		for ($i = 0; $i < 24; $i++) {
			$hour = sprintf("%02d", $i);
			if (isset($HData[$i])) { $j = $HData[$i]; } else { $j = 0; }
	...
	foreach ($DefConf as $cfgname => $cfgvalue) {
		if (!isset($CONF[$cfgname])) {
			if (!preg_match('/^[A-Za-z0-9\-_]+$/', $cfgname)) { continue; }
			if (!preg_match('/^[A-Za-z0-9\-_\s\.]*$/', $cfgvalue)) { continue; }
	...
	if (function_exists('glob')) {
		...
		foreach ($blockcachefiles as $f) { @unlink($f); }
	}
	return true;
}
?>