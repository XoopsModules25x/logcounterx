...
//	Get Configuration
$CONF = array();
$IgnoreHost = array(); $IgnoreReferer = array();
...
if (is_object($xoopsUser)) {
	if ($xoopsUser->IsAdmin()) { $usr = 2; $CONF['DATA_LIMIT'] = $CONF['REP_ROWLIMIT_ADM']; }
	else { $usr = 1; $CONF['DATA_LIMIT'] = $CONF['REP_ROWLIMIT_USR']; }
} else { $usr = 0; $CONF['DATA_LIMIT'] = $CONF['REP_ROWLIMIT']; }
$CONF['MyUserType'] = $usr;
...
$limit = intval($CONF['LOG_LIMIT']);
if ((0 < $limit) && ($limit < intval($log_count))) {
...
if (($usr >= $CONF['REP_RC']) || ($usr >= $CONF['REP_DR'])) {
	...
	$MaxDayCount = intval($MaxDayCount);
}

//	Recent Days
if (($MaxDayCount > 0) && ($usr >= $CONF['REP_RC'])) {
	$MyData = array(); $RecCount = 0;
	...
	while (list($MyName, $MyCount, $MyRobot) = $xoopsDB->fetchRow($res)) {
		...
		$MyRobot = intval($MyRobot);
		$MyDisplayName = htmlspecialchars($MyName);
		$MyData[$RecCount++] = array('Name' => $MyDisplayName, 'Count' => $MyCount, 'FormattedCount' => number_format($MyCount),
...
if ($usr >= $CONF['REP_WD']) {
	...
	for ($i = 0; $i < 7; $i++) {
		...
		for ($j = 0; $j < 7; $j++) {
			if (isset($MyTempData[$j]['Name']) && ($MyTempData[$j]['Name'] == strval($i))) { $MyData[$i] = $MyTempData[$j]; }
		}
	}
...
if ($usr >= $CONF['REP_TM']) {
	...
	while (list($MyName, $MyCount, $MyRobot, $Dummy) = $xoopsDB->fetchRow($res)) {
		...
		$SumCount += $MyCount;
		if ($MaxDayCount < $MyCount) { $MaxDayCount = $MyCount; }
	}
...
if ($usr >= $CONF['REP_RF']) {
	...
	foreach ($MyTempData as $Data) {
		...
		if (extension_loaded('mbstring')) {
			...
			if (mb_strwidth($MyName) > Lcx_ShortLength) { $MyShortName = mb_strimwidth($MyName, 0, Lcx_ShortLength - 3, ' ..'); }
			else { $MyShortName = $MyName; }
		} else {
			if (strlen($MyName) > Lcx_ShortLength) { $MyShortName = substr($MyName, 0, Lcx_ShortLength - 3).' ..'; }
			else { $MyShortName = $MyName; }
		}
...
function set_log_data ($sql, $limit = 24, $imgw = 0) {
	global $xoopsDB, $CONF;
	$MyData = array(); $RecCount = 0; $MaxCount = 0; $SumCount = 0;
	if ($limit == 0) { $limit = $CONF['DATA_LIMIT']; }
	if ($imgw == 0) { $imgw = $CONF['MAX_WIDTH']; }
	$res = $xoopsDB->query($sql, $limit);
	while (list($MyName, $MyCount, $dummy) = $xoopsDB->fetchRow($res)) {
		$MyCount = intval($MyCount); $MyLink = '';
